...
|------|-------------|-----------|
| `results.tsv` | Every experiment: commit, reward, status, description | Appends |
| `run.log` | stdout+stderr of the **last** evaluation only | Overwritten |
| `agent.py` | The current best agent code | Via git |
| `videos/` | Gameplay recordings of each **kept** experiment | Accumulates |
...
1. **Evaluation completes normally** — `agent.py` runs `train()` (up to 300s), then plays 30 episodes (up to 10,000 steps each). The process exits, `mean_reward` is parsed from stdout.
2. **The subprocess crashes** — any Python error (syntax, runtime, OOM). Non-zero exit code → logged as "crash".
3. **Timeout (10 minutes)** — if the subprocess hasn't exited after 600s, it's killed. Logged as "crash".
...
videos/exp0008_reward7-episode-0.mp4      # first improvement
videos/exp0095_reward186-episode-0.mp4    # DQN breakthrough
```
...
with open("results.tsv") as f:
    for row in csv.DictReader(f, delimiter="\t"):
        experiments.append({"reward": float(row["mean_reward"]), "status": row["status"]})

fig, axes = plt.subplots(2, 2, figsize=(14, 10))
fig.suptitle("AutoResearch Run Analysis", fontsize=14, fontweight="bold")

colors = {"keep": "#4CAF50", "discard": "#FF9800", "crash": "#f44336"}

ax = axes[0][0]
for i, e in enumerate(experiments):
    ax.scatter(i, e["reward"], c=colors[e["status"]], s=10, alpha=0.7)
ax.set_title("All Experiments"); ax.set_xlabel("Experiment #"); ax.set_ylabel("Reward")

ax = axes[0][1]
kept = [(i, e["reward"]) for i, e in enumerate(experiments) if e["status"] == "keep"]
ax.plot([k[0] for k in kept], [k[1] for k in kept], "g-o", ms=4)
ax.set_title("Kept (Monotonic Improvement)"); ax.set_xlabel("Experiment #"); ax.set_ylabel("Reward")

ax = axes[1][0]
counts = {s: sum(1 for e in experiments if e["status"] == s) for s in colors}
ax.bar(counts.keys(), counts.values(), color=[colors[s] for s in counts])
ax.set_title("Outcome Distribution")

ax = axes[1][1]
w = 20
cr = [sum(1 for e in experiments[i-w:i] if e["status"]=="crash")/w for i in range(w, len(experiments))]
ax.plot(range(w, len(experiments)), cr, "r-", alpha=0.7)
ax.set_title(f"Crash Rate (rolling {w})"); ax.set_ylim(0,1)

plt.tight_layout(); plt.savefig("run_analysis.png", dpi=150); plt.show()
```
...
gantt
    ...
    section Video
    Record if kept           :v1, 454, 470

    section Decision
    Compare + log            :e1, 470, 472
    Keep or reset            :e2, 472, 474
```
